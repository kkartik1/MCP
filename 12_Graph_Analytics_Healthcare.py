...
class DataLoader:
    """Module for loading and preprocessing graph data"""
    ...
    def load_for_graph_analysis(self) -> pd.DataFrame:
        """Load data specifically for graph construction"""
        query = """
        SELECT 
            claim_id,
            patient_id,
...
class HealthcareGraph:
    ...
    def build_tripartite_graph(self, df: pd.DataFrame) -> nx.Graph:
        ...
        for _, row in provider_patient.iterrows():
            G.add_edge(f"P_{row['provider_id']}", f"PT_{row['patient_id']}",
                      ...
                      upcoded_count=row['is_upcoded_line'])
    ...
    def build_provider_referral_network(self, df: pd.DataFrame) -> nx.DiGraph:
        ...
        print("\nBuilding provider referral network...")
        
        # Group by patient and get their providers in chronological order
        patient_providers = df.sort_values('service_date_line').groupby('patient_id')['provider_id'].apply(list)
...
class ReferralPatternAnalyzer:
    ...
    def detect_referral_loops(self) -> List[List[str]]:
        ...
        print("\nDetecting circular referral patterns...")
    ...
    def analyze_supply_chains(self, min_chain_length: int = 3) -> List[List[str]]:
        ...
        """
        print(f"\nAnalyzing supply chains (min length: {min_chain_length})...")
...
class ResultsExporter:
    ...
    def export_centrality_metrics(self, centrality_df: pd.DataFrame, 
        ...
        print(f"\nCentrality metrics exported to: {filepath}")
        return filepath
    ...
    def visualize_embeddings_2d(self, embed_df: pd.DataFrame):
        ...
        print("\nGenerating embedding visualization (t-SNE)...")
        
        # Get embedding columns
        embed_cols = [col for col in embed_df.columns if isinstance(col, int)]
...
class FraudSignalDetector:
    ...
    def detect_unusual_connectivity(self, threshold_percentile: float = 95) -> pd.DataFrame:
        ...
        """
        print("\nDetecting unusual connectivity patterns...")
    ...
    def detect_isolated_cliques(self, min_size: int = 4) -> List[Set[str]]:
        ...
        """
        print(f"\nDetecting isolated cliques (min size: {min_size})...")
    ...
    def detect_star_topology(self, min_spokes: int = 10) -> pd.DataFrame:
        ...
        """
        print(f"\nDetecting star topologies (min spokes: {min_spokes})...")
    ...
    def detect_unusual_procedure_combinations(self, min_support: int = 3) -> pd.DataFrame:
        ...
        """
        print("\nDetecting unusual procedure combinations...")
        ...
        # Find rare procedure combinations
        all_procedures = set()
        for procs in provider_procedures.values():
            all_procedures.update(procs)
        ...
        # Find rare but existing combinations
        rare_pairs = [(pair, count) for pair, count in pair_counts.items() 
                     if min_support <= count <= len(provider_procedures) * 0.05]
...
def main(db_path: str, max_nodes_for_embedding: int = 5000):
    ...
    print("\n[3/9] Computing Centrality Metrics...")
    print("="*100)
    ...
    if len(centrality_anomalies) > 0:
        print(f"\nDetected {len(centrality_anomalies)} nodes with anomalous centrality")
    
    # 4. Community Detection
    print("\n[4/9] Detecting Communities...")
    print("="*100)
    ...
    print("\n[5/9] Training Node2Vec Embeddings...")
    print("="*100)
    ...
    print("\n[6/9] Analyzing Referral Patterns...")
    print("="*100)
    ...
    if len(unusual_connectivity) > 0:
        print(f"\nFound {len(unusual_connectivity)} nodes with unusual connectivity patterns")
    
    # Isolated cliques
    isolated_cliques = fraud_detector.detect_isolated_cliques(min_size=4)
    if isolated_cliques:
        print(f"\nFound {len(isolated_cliques)} isolated cliques (potential collusion)")
    
    # Star topologies
    star_topologies = fraud_detector.detect_star_topology(min_spokes=10)
    if len(star_topologies) > 0:
        print(f"\nFound {len(star_topologies)} potential star topology patterns")
        print("\nTop 5 Star Patterns:")
    ...
    if len(unusual_procedures) > 0:
        print(f"\nFound {len(unusual_procedures)} unusual procedure combinations")
    
    # 8. Export Results
    print("\n[8/9] Exporting Results...")
    print("="*100)
    ...
    print(f"\nAll outputs saved to './outputs/graph_analysis' directory")
    print("="*100)
